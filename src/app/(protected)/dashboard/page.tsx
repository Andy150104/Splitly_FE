...
export default async function DashboardPage() {
  const permissionState = await getCurrentPermissionState();
  ...
  if ("error" in loaded) {
    return (
      <div className="space-y-8">
        <PageHeader
          title="Tổng quan"
          description="Không thể lấy dữ liệu mới nhất từ hệ thống."
        />
        <ErrorState
          message={
    ...
    <div className="space-y-8">
      ...
      {canReadBills ? (
        <section>
          ...
          <BillList bills={ownedItems} canCreate={canWriteBills} />
        </section>
      ) : null}
    </div>
  );
