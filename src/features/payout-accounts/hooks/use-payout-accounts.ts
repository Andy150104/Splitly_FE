...
import type { PayoutAccount, VietQrBank } from "@/features/payout-accounts/types";
...
export function useCreatePayoutAccount() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: payoutAccountQueryKeys.all });
    },
    onError: (error) => {
      toast.error(
        error instanceof Error ? error.message : "Không thể thêm tài khoản ngân hàng.",
      );
    },
  });
}
